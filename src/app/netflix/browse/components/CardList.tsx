        ...
        <div className={styles['card-list']}>
            <div className={styles['pagination-wrap']}>
                {Array.from({ length: totalPages }, (_, index) => (
                    <span key={index} className={`${styles.pagination} ${index === currentItem ? styles.active : ''}`}></span>
                ))}
            </div>
            <h3>{data.title}</h3>
            <div className={styles.contents}>
                ...
                <span className={`${styles['btn-prev']} ${styles['btn-wrap']}`}>
                    <button type="button" onClick={handlePrev} style={currentItem === 0 ? { display: 'none' } : {}}>
                        <span className="blind">이전</span>
                    </button>
                </span>

                <span className={`${styles['btn-next']} ${styles['btn-wrap']}`}>
                    <button type="button" onClick={handleNext} style={currentItem === totalPages - 1 ? { display: 'none' } : {}}>
